...
import cleanupDescargar from "./cleanupDescargar.js";
import updateAddonInstalarStats from "./updateAddonInstalarStats.js";
const GITHUB_ACCESS_TOKEN = `${GITHUB_TOKEN}`;
/**
 * Handles the installation of an addon
 */
const handleAddonInstalaration = async (
  addon,
  event,
  isReinstall = false,
  ...
  try {
    ...
    setInstalaringAddonStep("Deleting previous folders");
        ...
        if (installedAddon) {
          ...
          await Promise.all(
            foldersToDelete.map(async (folder) => {
              ...
              let deleteCorrecto = false;
              for (let attempt = 0; attempt < 2; attempt++) {
                await window.electron.deleteFolder(folderPath);
                setInstalaringAddonStep(`Deleting old folders`);
    ...
    // ----------------------------------------------------------------------------------
    // Step 3: Descargar and extract the new addon
    // IMPORTANT: responseType is "blob" so .arrayBuffer() works in Electron
    // ----------------------------------------------------------------------------------
    setInstalaringAddonStep("Descargaring addon...");
    ...
    if (websiteLink.includes("github.com")) {
      try {
        ...
        // 2) Descargar the repository zip from default branch
        ...
        try {
          ...
          console.warn(
            "[Warperia-TDLO] HEAD request failed. Falling back to unknown size."
          );
        }

        // 3) Descargar the repository zip from default branch
    ...
    const beforeInstalarItems = await window.electron.readDir(installPath);
    ...
    // ----------------------------------------------------------------------------------
    setInstalaringAddonStep("Restructuring folder...");
    ...
    const afterInstalarItems = await window.electron.readDir(installPath);
    const newlyExtractedFolders = afterInstalarItems.filter(
      (item) => !beforeInstalarItems.includes(item)
    );
    ...
    if (newlyExtractedFolders.length === 1) {
        ...
        if (extractedFolderName !== mainFolderName) {
          ...
          try {
            ...
            if (subfolders.length === 1 && subfolders[0] === mainFolderName) {
              console.log(
                `[Warperia-TDLO] Found a double folder: ${mainFolderName}\\${mainFolderName}. Flattening...`
              );
      ...
      try {
        ...
        if (extractedFolderName === mainFolderName) {
          ...
          if (subfolders.length === 1 && subfolders[0] === mainFolderName) {
            console.log(
              `[Warperia-TDLO] Detected a double folder: ${mainFolderName}/${mainFolderName}. Flattening...`
            );
    ...
    // ----------------------------------------------------------------------------------
    setInstalaringAddonStep("Cleaning up...");
    ...
    if (await window.electron.fileExists(tocFilePath)) {
      ...
      const wordPressVersionAtTimeOfInstalar =
      ...
      const warperiaContent = `ID: ${
        ...
        .join(
          ","
        )}\nBackendVersion: ${wordPressVersionAtTimeOfInstalar}\nFilename: ${backendFilename}\n${
      ...
      // Compare versionFromToc vs. wordPressVersionAtTimeOfInstalar
      let finalTocVersion = versionFromToc;
      const compareResult = semverCompare(
        versionFromToc,
        wordPressVersionAtTimeOfInstalar
      );
      if (compareResult < 0) {
        finalTocVersion = wordPressVersionAtTimeOfInstalar;
      }
...
export default handleAddonInstalaration;