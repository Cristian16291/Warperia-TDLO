...
const AddonModal = ({ show, onHide, addon, loading, installedAddon, downloading, installingAddonId, installingAddonStep, progress, onInstalar, onReinstall }) => {
    const [lightboxOpen, setLightboxOpen] = useState(false);
    const [currentImage, setCurrentImage] = useState(null);
    const [activeTab, setActiveTab] = useState('description');
    const [developers, setDevelopers] = useState([]);
    const modalRef = useRef(null); // Reference to the modal content
    const isInstalared = !!installedAddon;
    const isInstalaring = downloading && installingAddonId === addon?.id;
                                        ...
                                        <div className="install-controls">
                                            {downloading && installingAddonId === addon.id ? (
                                                ...
                                                <button
                                                    ...
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        installedAddon ? onReinstall(addon, e) : onInstalar(addon, e);
                                                    }}
                                                    disabled={downloading}
                                                >
                                                    {installedAddon ? 'Reinstall' : 'Instalar'}
                                                    {downloading && installingAddonId === addon.id && (
