          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">
                {isInstalaration && newAddon
                  ? `Confirm Instalaration`
                  : `Confirm Deletion`}
              </h5>
              <button
                type="button"
            ...
            <div className="modal-body">
              ...
              {parentAvisos.length > 0 && (
                <div className="alert alert-medium">
                  <p className="mb-1 fw-medium text-warning"><i className="bi bi-exclamation-circle-fill me-1"></i> Aviso!</p>
                  <small>
                    This addon is bundled into the following addon(s):
                    {parentAvisos.map((p, idx) => (
                      <span key={p.id}> {p.title}{idx < parentAvisos.length - 1 ? ',' : ''}</span>
                    ))}
