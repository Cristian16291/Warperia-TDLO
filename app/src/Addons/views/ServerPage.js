...
const ServerPage = ({ user }) => {
  ...
  // -------------------------------------------
  useEffect(() => {
    const handleProcessStatusActualizar = (status) => {
      if (!server) return;
  ...
  useEffect(() => {
    const detectRealmlist = async () => {
      ...
      try {
        ...
        const realmlistFilePath = await window.electron.pathJoin(
          dataDir,
          locale,
          "realmlist.wtf"
        );
        ...
        if (isMop) {
          const configPath = await window.electron.pathJoin(
            dirPath,
            "WTF",
            "Config.wtf"
          );
          ...
          if (configExists) {
            ...
            configRealmlistValue = parseRealmlistFromContent(
              configContent,
              true
            );
          }
        ...
        <div className="content page-addons flex-grow-1">
          <div className="container">
            <div className="my-4">
              ...
              <p>
                The server you're trying to access does not exist or has been
                removed.
              </p>
            </div>
          </div>
                    ...
                    <div className="d-flex align-items-center gap-2 mt-3 control-buttons">
                      ...
                      >
                        ...
                        >
                          ...
                          {isRunning ? " Running" : " Iniciar"}
                        </button>
                      </Tippy>
                      <Tippy
                        content="Reiniciar game"
                        placement="bottom"
                        className="custom-tooltip"
                      >
                        <button
