...
const Acceder = ({ onAcceder }) => {
  const [usernameOrEmail, setUsuarioOrEmail] = useState("");
  ...
  const handleAcceder = async (e) => {
    ...
    try {
      ...
      if (response.data.token) {
        ...
        onAcceder(response.data.user);
      } else {
        setMessage("Acceder failed. Please check your credentials.");
      }
    } catch (error) {
      // Handle errors
      if (error.response) {
