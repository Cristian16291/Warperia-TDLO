...
const Sidebar = () => {
  const [updateUrl, setActualizarUrl] = useState("");
  const [appVersion, setAppVersion] = useState("");
  const [latestVersion, setLatestVersion] = useState("");
  const [userServidores, setUserServidores] = useState([]);
  const [settings, setAjustes] = useState(null);
  ...
  const fetchUserServidores = async () => {
    try {
      if (!user || !user.id) {
        console.warn("User is not defined or missing 'id'. Skipping fetchUserServidores.");
        return;
      }
  ...
  useEffect(() => {
    ...
    const fetchAjustes = fetch(`${WEB_URL}/wp-json/wp/v2/site-settings`).then((res) => res.json());

    Promise.all([fetchAppVersion, fetchAjustes])
      .then(([versionResult, settingsData]) => {
        setAppVersion(versionResult.version);
        setLatestVersion(settingsData.app_version);
        setActualizarUrl(settingsData.app_file);
      })
      .catch((error) => {
  ...
  return (
    <><div className="sidenav d-flex flex-column py-3 text-white">
      ...
      <ul className="nav nav-pills flex-column mb-auto">
        ...
        <Tippy content="Servidores" placement="right" className="custom-tooltip">
          <li className="nav-item mb-2">
            <NavLink
              to="/servers"
              className={({ isActive }) => `nav-link text-white text-center ${isActive ? 'active' : ''}`}
      ...
      <div className="sidenav-bottom w-100 mb-3">
        <ul className="nav nav-pills flex-column mb-auto">
          ...
          <Tippy content="Ajustes" placement="right" className="custom-tooltip">
            <li className="nav-item">
              <NavLink
                to="/settings"
                className={({ isActive }) => `nav-link text-white text-center ${isActive ? 'active' : ''}`}
      ...
      {showAddServerModal && (
        <Suspense fallback={<div className="text-center my-4"><div className="spinner-border" role="status"><span className="visually-hidden">Loading...</span></div></div>}>
          <AddServerModal
            ...
            user={user}
            refreshServidores={fetchUserServidores}
          />
        </Suspense>
      )}
    </>
