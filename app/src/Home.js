...
const Home = ({ user, setPage }) => {
  ...
  const [servers, setServidores] = useState([]);

  // Loading state
  const [loading, setLoading] = useState(true);

  // State for user's servers
  const [userServidores, setUserServidores] = useState([]);
  const [userServidoresLoading, setUserServidoresLoading] = useState(false);
  ...
  useEffect(() => {
    const fetchUserServidores = async () => {
      try {
        if (!user || !user.id) return;
        setUserServidoresLoading(true);
        ...
        setUserServidoresLoading(false);
      }
    };
  ...
  return (
    <div className="d-flex homepage h-100">
      ...
      <div className="content flex-grow-1">
        ...
        <section className="container border-bottom border-dark pb-5">
          <div className="col-lg-6">
            ...
            <p className="text-muted">
              Warperia-TDLO is a new seamless way of browsing and installing addons
        ...
        <section className="container statistics py-5">
          ...
          <div className="d-flex align-items-center justify-content-between">
            <div className="card servers">
              <div className="value">{userServidores?.length || 0}</div>
              <div className="text text-muted">Servidores</div>
            </div>
            <div className="card servers">
          ...
          userServidores.length > 0 && (
            <section className="container py-5">
              ...
              <div className="row row-cols-1 row-cols-md-2 align-items-center g-4">
                {userServidores.slice(0, 4).map((server, index) => (
        ...
        {/* Public Servidores */}
        <section className="container py-5">
          <h2 className="text-white fw-bold fs-5">Private Servidores</h2>
